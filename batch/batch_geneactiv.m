...
%     format)
%   - ./txt will save all the sleep results from the every csd file

% CHANGE THIS: data folder
datapath = 'D:\data\recordings\pup2\';

addpath(genpath('D:\tresorit\matlab\actant-1'));
...
csd_files = dir('./csd/*.csv');

% check the number of files in the csd folder, which will be limiting
% factor
...
for file = 1:numel(csd_files)
    ...
    fid = fopen(csd_files(file).name, 'r');
    ...
    fid = fopen(['./txt/' csd_files(file).name(1:end-4) '.txt'], 'w');
